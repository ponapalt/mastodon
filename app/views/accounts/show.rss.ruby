  ...
  @statuses.each do |status|
    doc.item do |item|
      ...
      if status.ordered_media_attachments.first&.audio?
        ...
        item.enclosure(origin_asset_url(media.file.url(:original, false)), media.file.content_type, media.file.size)
      end

      status.ordered_media_attachments.each do |media_attachment|
        item.media_content(origin_asset_url(media_attachment.file.url(:original, false)), media_attachment.file.content_type, media_attachment.file.size) do |media_content|
